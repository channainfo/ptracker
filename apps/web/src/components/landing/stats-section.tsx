      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mb-16">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-foreground mb-4">Trusted by the Crypto Community</h2>
            <p className="text-muted-foreground">Join thousands of successful investors already using our platform</p>
          </div>
          
          <dl className="grid grid-cols-1 gap-6 text-center lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.name} className="bg-card p-8 rounded-2xl border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10">
                <dt className="text-sm font-medium text-muted-foreground mb-2">{stat.name}</dt>
                <dd className={`text-4xl font-bold ${stat.color} mb-2`}>
                  {stat.value}
                </dd>
                <div className="flex items-center justify-center gap-1 text-sm">
                  <ArrowUp className="h-4 w-4 text-green-500" />
                  <span className="text-green-500">{stat.change} this month</span>
                </div>
              </div>
            ))}
          </dl>
        ...
        <div className="bg-card rounded-2xl p-8 border border-border">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">Real-time prices updated every second</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {marketData.map((crypto) => (
              <div key={crypto.symbol} className="bg-muted/30 p-6 rounded-xl border border-border hover:border-primary/50 transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="font-bold text-foreground">{crypto.price}</div>
                ...
                <div className="w-full bg-muted rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${
                      crypto.changeType === 'positive' ? 'bg-green-500' : 'bg-red-500'
                    }`}
        ...
        <div className="mt-16 text-center">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 items-center opacity-70">
            <div className="text-center">
              ...
              <div className="text-foreground font-semibold">Bank-Level Encryption</div>
            </div>
            <div className="text-center">
              <div className="text-sm text-muted-foreground mb-2">Uptime</div>
              <div className="text-foreground font-semibold">99.9% Availability</div>
            </div>
            <div className="text-center">
              <div className="text-sm text-muted-foreground mb-2">Response Time</div>
              <div className="text-foreground font-semibold">&lt;100ms Average</div>
            </div>
            <div className="text-center">
              <div className="text-sm text-muted-foreground mb-2">Support</div>
              <div className="text-foreground font-semibold">24/7 Live Chat</div>
            </div>
          </div>
        </div>
      </div>
